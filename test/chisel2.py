...
DIRECTORY = os.getenv('DIRECTORY', 'https://dev.xiexianbin.cn:14000/dir')
...
def wait_for_acme_server():
    ...
    while True:
        try:
            if requests.get(DIRECTORY, verify=False).status_code == 200:
                return
...
def make_client(email=None):
    ...
    net = acme_client.ClientNetwork(key, verify_ssl=False, user_agent="x-ca foo-acme tester")
...
if __name__ == "__main__":
    ...
    try:
        ...
        auth_and_issue(domains, email="me@xiexianbin.cn")
    except messages.Error as e:
        print(e)
        sys.exit(1)
